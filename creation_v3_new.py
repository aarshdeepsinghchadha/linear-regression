...
output_dir = os.path.join(root_dir, 'data', 'v3')  # C:\Project\Self\prophet\data\v3

# Create 'data/v3' folder at the root level if it doesn't exist
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Projects (12 projects as per requirement)
projects = pd.DataFrame({
    'projectid': range(1, 13),
...
# Users (50 users as per requirement)
    ...
    for month in range(1, 13):
        ...
        for username in users:
            ...
            if behavior == 'no_log':
                ...
                total_hours = 160
                num_projects = random.randint(1, 12)  # Between 1 and 12 projects
                ...
                num_projects = random.randint(1, 12)
                num_days = min(random.randint(10, days), days)  # Fewer days than full-time

            # Select random projects from the 12 available
            selected_project_ids = random.sample(range(1, 13), num_projects)
...
# Save files to the 'data/v3' directory at the root level
timelogs.to_csv(os.path.join(output_dir, 'user_timelogs_v3.csv'), index=False)
projects.to_csv(os.path.join(output_dir, 'projects_v3.csv'), index=False)
