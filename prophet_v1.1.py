...
# Save forecasts
user_forecasts.to_csv(f'{output_folder}/csv_files/user_forecasts_2025_v8.csv', index=False)
project_forecasts.to_csv(f'{output_folder}/csv_files/project_forecasts_2025_v8.csv', index=False)
...
top_users_historical.to_csv(f'{output_folder}/csv_files/historical_top_users_2010_2024_v8.csv', index=False)
...
top_users_2025_pred.to_csv(f'{output_folder}/csv_files/predicted_top_users_2025_v8.csv', index=False)

# --- Graphing Section ---

# 1. Historical Top Users (2010-2024, split into 5-year periods)
for start_year in range(2010, 2025, 5):
    ...
    plt.title(f'Historical Top User Hours by Month ({start_year}-{end_year})')
    plt.xlabel('Month')
    ...
    plt.tight_layout()
    plt.savefig(f'{output_folder}/images/historical_top_users_{start_year}_{end_year}_v8.png')
    plt.close()

# 2. Historical Total Project Hours (2010-2024, split into 5-year periods)
...
for start_year in range(2010, 2025, 5):
    ...
    for year in range(start_year, end_year + 1):
        ...
        plt.plot(year_data['month'], year_data['timelog'], label=str(year), marker='o')
    plt.title(f'Historical Total Project Hours by Month ({start_year}-{end_year})')
    plt.xlabel('Month')
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_folder}/images/historical_project_hours_{start_year}_{end_year}_v8.png')
    plt.close()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(f'{output_folder}/images/predicted_top_users_2025_v8.png')
plt.close()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(f'{output_folder}/images/predicted_project_hours_2025_v8.png')
plt.close()

# --- Output Summary ---
print(f"\nCSVs saved in '{output_folder}/csv_files':")
print("  - user_forecasts_2025_v8.csv")
print("  - project_forecasts_2025_v8.csv")
print("  - historical_top_users_2010_2024_v8.csv")
print("  - predicted_top_users_2025_v8.csv")
print(f"Images saved in '{output_folder}/images':")
print("  - historical_top_users_2010_2014_v8.png")
print("  - historical_top_users_2015_2019_v8.png")
print("  - historical_top_users_2020_2024_v8.png")
print("  - historical_project_hours_2010_2014_v8.png")
print("  - historical_project_hours_2015_2019_v8.png")
print("  - historical_project_hours_2020_2024_v8.png")
print("  - predicted_top_users_2025_v8.png")
print("  - predicted_project_hours_2025_v8.png")